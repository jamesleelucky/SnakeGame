...
# # Hand detector optimized for speed (MediaPipe uses GPU on macOS)
...
# # -----------------------------
# prev_time = time.time()
# fps_list, latency_list = deque(maxlen=100), deque(maxlen=100)
...
# frame_skip = 2  # Detect every 2nd frame for FPS boost
...
#         hands, _ = detector.findHands(small_frame, flipType=False)
#     else:
#         hands = []
...
#         pointIndex = (int(lmList[8][0] * 2), int(lmList[8][1] * 2))  # Scale back to full size

#         # Detect movement
#         if previous_position:
#             movement = abs(pointIndex[0] - previous_position[0]) + abs(pointIndex[1] - previous_position[1])
#             if movement > 30 and not pending_marker:
#                 movement_detected_time = time.time()
#                 pending_marker = True
#                 pending_frame = total_frames + 1  # Wait until next frame to draw
#         img = game.update(img, pointIndex)
#         previous_position = pointIndex

#     # Show marker and calculate REAL latency
#     if pending_marker and total_frames >= pending_frame:
#         cv2.rectangle(img, (0, 0), (80, 80), (0, 0, 255), -1)  # Visual marker
...
#     cv2.putText(img, f'Latency: {avg_latency:.1f} ms', (10, 70),
...
# print(f"Average REAL Latency: {avg_latency:.2f} ms")
